"""
Обработчики команд и сообщений для авторизованных пользователей.
"""
import asyncio
import re
import logging
...
from api import chat_completion, get_balance, pick_models_in_order, validate_api_key
...
from utils import split_long_message, ts_to_time, is_authorized, is_admin, clean_and_format_ai_response
...
async def help_handler(message: Message):
    ...
    try:
        # ИЗМЕНЕНО: Описание команды /reset обновлено.
        help_text = (
...
async def reset_handler(message: Message, state: FSMContext):
    ...
    try:
        # ИЗМЕНЕНО: Текст подтверждения теперь не упоминает сброс бюджета.
...
async def confirm_reset(message: Message, state: FSMContext):
    ...
    try:
        # ИЗМЕНЕНО: Убран вызов reset_daily(user_id) для сохранения бюджета.
        user_cfg = load_user_config(user_id)
        ...
        # ИЗМЕНЕНО: Сообщение об успехе теперь не упоминает сброс бюджета.
...
# Обработка неожиданного ввода во время ожидания подтверждения
...
async def budget_handler(message: Message):
    ...
    try:
        ...
        # Базовый текст с токенами — для всех
        text = (
        ...
        # Добавляем баланс OpenRouter ТОЛЬКО для администраторов
        if is_admin(user_id):
